...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    updateAudioInfo(state, payload) {
      console.log('state.audioInfo.length: ', state.audioInfo.length);
      if (state.audioInfo.length == 3) {
  ...
  actions: {
    // 获取音乐地址
    async getAudioInfo(context, id) {
      let result = {}

      Promise.all([
        context.dispatch('getMusicUrl', id),
        context.dispatch('getMusicInfo', id),
        context.dispatch('getMusicLyric', id)
      ]).then(res => {
        console.log('res: ', res);
        // id 是用来区分音频的唯一标识
